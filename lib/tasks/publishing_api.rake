...
namespace :publishing_api do
  ...
  namespace :bulk_republish do
    ...
    desc "⚠️  WARNING: this rake task republishes all draft documents with HTML attachments ⚠️. Republish all draft documents with HTML attachments to the Publishing API."
    task draft_html_attachments: :environment do
      document_ids = Edition
        .draft
        .where(id: HtmlAttachment.where(attachable_type: "Edition").select(:attachable_id))
        .pluck(:document_id)
